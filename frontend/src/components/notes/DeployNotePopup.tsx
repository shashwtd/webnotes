...
export function DeployNotePopup({ isOpen, onClose }: Props) {
    ...
    const { recentNotes, loading } = useNotes();
    const { handleDeploy: deployNote } = useDeployment();

    const matchingNotes = recentNotes
        .filter(note => !note.deployed && note.title.toLowerCase().includes(searchTerm.toLowerCase()))
        .slice(0, 5);
                ...
                <div className="fixed inset-0 bg-black/50 flex items-start justify-center pt-20 z-50">
                    ...
                    >
                        ...
                        <div className="p-6 flex flex-col gap-4">
                            <div>
                                <h2 className="text-xl font-semibold">Deploy a Note</h2>
                                                    ...
                                                    {matchingNotes.map(note => (
                                                        <button
                                                            ...
                                                            className={`w-full px-4 py-2 text-left hover:bg-neutral-50 ${
                                                                selectedNote?.id === note.id ? "bg-neutral-50" : ""
                                                            }`}
                                                        >
